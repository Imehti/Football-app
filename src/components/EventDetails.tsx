      ...
      {eventDetails && (
        <>
          ...
          <div className="flex flex-col mt-2 border bg-blue-200">
            ...
            {eventDetails.event.map((e) => e.strStatus === "Not Started") && (
              <div className="flex justify-center items-center">
                <h1 className="font-bold sm:text-2xl">
                  {eventDetails.event.map((e) => {
                    return (
                      <div className="flex justify-between">
                        {e.intHomeScore && `${e.intHomeScore} :`}{" "}
                        {e.intAwayScore}
                      </div>
                    );
                  })}
            ...
            <div className="m-2 grid grid-cols-1 sm:grid-cols-2">
              <div>
                ...
                {eventDetails.event?.some((e) => e.strVenue !== null) && (
                  <h1 className="font-bold">
                    Stadium :{" "}
                    {
                      eventDetails.event.find((e) => e.strVenue !== null)
                        ?.strVenue
                    }
                  </h1>
                )}
                {eventDetails.event?.some((e) => e.intSpectators !== null) && (
                  <h1 className="font-bold">
                    Spectators :{" "}
                    {
                      eventDetails.event.find((e) => e.intSpectators !== null)
                        ?.intSpectators
                    }
                  </h1>
                )}
                {eventDetails.event.some(
                  (e) =>
                    e.strDescriptionEN !== "" && e.strDescriptionEN !== null
                ) && (
                  <h1 className="font-bold">
                    Description :{" "}
                    {
                      eventDetails.event.find((e) => e.strDescriptionEN !== "")
                        ?.strDescriptionEN
                    }
                  </h1>
                )}
